...
int recovery_notcheck_display = 0;// add by cjcheng for display_param
...
//	printf("misc.recovery : %s\n", misc_message.recovery);
	...
	if(storage_type)
	{
		if(!strcmp(misc_message.command, "boot-recovery")) {
			...
			recovery_notcheck_display = 1;// add by cjcheng for display_param
...
{
	sunxi_mmc_init(CONFIG_MMC_SUNXI_SLOT); // modify by cjcheng for display_param
